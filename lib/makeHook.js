...
function makeHook(fileLimits, requiredFiles, {limits, preservePath}) {
  ...
  return function multipartHook(req, res, next) {
    ...
    const fileAccumulator = new FileAccumulator(fileLimits, abort);
    ...
    busboy.on('field', function onField(fieldName, value, fieldNameTruncated, valueTruncated) {
      ...
      if (fieldNameTruncated || fieldName.length > fieldNameSizeLimit) {
        abort(new MultipartError('LIMIT_FIELD_NAME'));
        return;
      }

      if (valueTruncated) {
        abort(new MultipartError('LIMIT_FIELD_VALUE', fieldName));
        return;
      }
    ...
    busboy.on('file', function onFile(fieldName, fileStream, fileName, transferEncoding, mimeType) {
      ...
      if (errorCode !== null) {
        fileStream.resume();
        abort(new MultipartError(errorCode, fieldName));
      }
    });

    busboy.on('error', abort);
    busboy.on('fieldsLimit', () => abort(new MultipartError('LIMIT_FIELD_COUNT')));
    busboy.on('filesLimit', () => abort(new MultipartError('LIMIT_FILE_COUNT')));
    busboy.on('partsLimit', () => abort(new MultipartError('LIMIT_PART_COUNT')));
    ...
    busboy.on('finish', () => {
      ...
      fileAccumulator.genFiles((files) => {
        // Ensure that all non-optional files are accounted for
